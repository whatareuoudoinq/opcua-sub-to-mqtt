...
class OPCUAWorker(threading.Thread):
    ...
    async def async_main(self):
        async with OpcuaClient(OPCUA_SERVER_URL) as client:
            # === [요약 정보] 서버명/네임스페이스/루트 노드/주소 전달 ===
            srv_url = OPCUA_SERVER_URL
            is_connected = True
            srv_name = "-"
            ns_array = []
            try:
                srv_name = await client.get_server_name()
            except:
                pass
            try:
            ...
            while self.should_run:
                for var in TARGET_VARIABLES:
                    ...
                    self.gui.update_opcua_summary_lastupdate(ts)
                time.sleep(1)
...
class IntegratedGUI(tk.Tk):
    def __init__(self):
        ...
        # ▼▼▼  [개선] OPC UA 요약 정보 표시 프레임 ▼▼▼
        self.opcua_summary = tk.LabelFrame(left, text="서버 요약", padx=8, pady=5)
        self.opcua_summary.pack(fill=tk.X, pady=(8,3), padx=2)
        self.lbl_server_url = ttk.Label(self.opcua_summary, text="Server: -")
        self.lbl_server_url.pack(anchor="w")
        self.lbl_server_name = ttk.Label(self.opcua_summary, text="Server Name: -")
        self.lbl_server_name.pack(anchor="w")
        self.lbl_ns_array = ttk.Label(self.opcua_summary, text="Namespace: -")
        self.lbl_ns_array.pack(anchor="w")
        self.lbl_root_id = ttk.Label(self.opcua_summary, text="Root NodeId: -")
        self.lbl_root_id.pack(anchor="w")
        self.lbl_last_update = ttk.Label(self.opcua_summary, text="Last Data Update: -")
        self.lbl_last_update.pack(anchor="w")
        # ▲▲▲
        ...
        # ▼▼▼ [개선] 요약 정보 업데이트 함수 추가 ▼▼▼
    def update_opcua_summary(self, connected, server_url, server_name, namespaces, root_id, last_data_update):
        self.lbl_server_url.config(
            text=f"Server: {server_url} {'[Connected]' if connected else '[Disconnected]'}")
        self.lbl_server_name.config(text=f"Server Name: {server_name}")
        self.lbl_ns_array.config(text=f"Namespace: {namespaces}")
        self.lbl_root_id.config(text=f"Root NodeId: {root_id}")
        self.lbl_last_update.config(text=f"Last Data Update: {last_data_update}")

    def update_opcua_summary_lastupdate(self, ts):
        self.lbl_last_update.config(text=f"Last Data Update: {ts}")
